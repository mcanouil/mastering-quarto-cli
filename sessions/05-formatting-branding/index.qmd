...
This session explores Quarto's branding capabilities, focusing on `brand.yml` as the central tool for unified branding across formats, the `quarto use brand` command for importing shared brand configurations, and the `brand.yml` R and Python packages for applying brand guidelines in code outputs.
...
- [Import and manage external brand configurations with `quarto use brand`.]{.fragment fragment-index="3"}
- [Use the `brand.yml` R and Python packages to apply brand guidelines in code outputs.]{.fragment fragment-index="4"}
