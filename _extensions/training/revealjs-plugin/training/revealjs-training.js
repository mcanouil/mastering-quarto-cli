...
window.RevealJsTraining = function () {
  "use strict";
  ...
  function processSectionSlides(cfg) {
    ...
    for (const section of allSections) {
      if (
        ...
        section.classList.contains("training-title-slide")
      ) {
        continue;
      }
  ...
  function updateTitleSlideChrome() {
    ...
    const isTitle =
      currentSlide?.classList.contains("training-title-slide") ||
  ...
  function applyDebugBorders() {
    ...
    for (const section of sections) {
      ...
      if (
        section.classList.contains("training-title-slide") ||
        section.classList.contains("training-closing-slide")
      ) {
        borderColour = "green";
  ...
  return {
    id: "training-revealjs",

    init: function (deck) {
      const deckConfig = deck.getConfig();

      // Read from extensions.training namespace
      const training = deckConfig["extensions"]?.["training"] || {};

      // Build config from extensions.training.* options
      config = {
        ...defaults,
        sectionOutline: training["section-outline"] ?? defaults.sectionOutline,
        dateSuperscript: training["date-superscript"] ?? defaults.dateSuperscript,
        faviconFromLogo: training["favicon-from-logo"] ?? defaults.faviconFromLogo,
        hideTitleSlideChrome:
          training["hide-title-slide-chrome"] ?? defaults.hideTitleSlideChrome,
        debugBorders: training["debug-borders"] ?? defaults.debugBorders,
      };
