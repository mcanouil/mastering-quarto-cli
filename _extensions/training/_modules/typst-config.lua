...
--- @type string Section name under extensions.training
...
local function get_builtin_mappings()
  return {
    div = {
      ['highlight'] = {
        wrapper = 'training-highlight',
        arguments = false
      },
      ['value-box'] = {
        wrapper = 'training-value-box',
        arguments = true
      },
      ['panel'] = {
        wrapper = 'training-panel',
        arguments = true
      },
      ['divider'] = {
        wrapper = 'training-divider',
        arguments = true
      },
      ['progress'] = {
        wrapper = 'training-progress',
        arguments = true
      },
      ['executive-summary'] = {
        wrapper = 'training-executive-summary',
        arguments = true
      },
      ['card-grid'] = {
        wrapper = 'training-card-grid',
        arguments = true
      },
      ['card'] = {
...
local function parse_and_validate_config(config, class)
  ...
  -- Check if it's a simple string value (e.g., 'highlight: training-highlight')
...
--- Reads configuration from document metadata under extensions.training.typst-markdown.
...
local function load_element_mappings(meta)
  ...
  -- Read configuration from extensions.training.typst-markdown
  local training_config = meta_mod.get_extension_config(meta, 'training')
  local extension_config = training_config and training_config[SECTION_NAME]
