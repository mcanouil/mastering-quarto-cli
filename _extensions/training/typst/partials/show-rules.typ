...
// Set by training-code-window wrapper, read by apply-code-block-style
#let code-window-filename = state("code-window-filename", none)

// Annotation state passed to Skylighting via Typst state
// Set by training-code-window wrapper when annotations are present
...
#let training-code-window-annote-colour(bg) = {
  if type(bg) == color {
    let comps = bg.components(alpha: false)
    let lum = if comps.len() == 1 {
      comps.at(0) / 100%
...
#let training-code-window-circled-number(n, bg-colour: none) = {
  let c = if bg-colour != none { training-code-window-annote-colour(bg-colour) } else { luma(120) }
...
#let training-code-window-annotation-item(block-id, n, content) = {
  ...
  context {
    ...
    [#block(above: 0.4em, below: 0.4em)[
      #if has-target {
        link(target)[#training-code-window-circled-number(n)]
      } else {
        training-code-window-circled-number(n)
      }
      #h(0.4em)
      #content
...
#let training-code-window-annotated-content(content, annotations: (:), bg-colour: none, block-id: 0) = {
...
#let apply-figure-style(it, colours) = {
  ...
  let is-styled = (
    body-repr.contains("training-featured-image")
